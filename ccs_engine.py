...
def phase1_qualified_fields(details: Dict[str, Dict[str, Any]]) -> List[str]:
    """Return only fields that passed the complete Phase-1 hard-cutoff screening.

    Phase-1 failures are permanently eliminated from Phase 2. This helper
    deliberately checks the stored ``Passed`` flag rather than relying on
    table ordering or on Phase-2 gate results.
    """
    return [
        field_name
