...
def phase1_qualified_field_names(details: Dict[str, Dict[str, Any]]) -> List[str]:
    """Return only fields that passed every Phase-1 screening requirement.

    This is the authoritative eligibility list for Phase 2. A field with
    Passed=False is permanently excluded from Phase 2.
    """
    return [
        field_name
        for field_name, result in details.items()
...
def project_cashflows(
    ...
    annual_revenue_year1 = tonnes * unit_revenue
    return npv, irr, payback, annual_revenue_year1, cashflows
...
def economic_analysis(fields, details, assumptions):
    ...
    for field in fields:
        r = details[field]
        npv, irr, payback, annual_revenue, cashflows = project_cashflows(
        ...
        raw[field] = {"Revenue (Year 1)": annual_revenue, "NPV": npv, "IRR": irr, "Payback Period": payback, "Cashflows": cashflows}
        rows.append({
            "Field": field,
            "Actual CAPEX": r["Actual CAPEX"],
            "Actual OPEX": r["Actual OPEX"],
            "Revenue (Year 1)": annual_revenue,
            "NPV": npv,
...
def sensitivity_oat(fields, details, base_assumptions, ranges):
    ...
    Each requested parameter is changed to its Low and High endpoint while all
    other economic assumptions remain at their field/base values. This produces
    2 scenarios per parameter and reports the actual extrema across all OAT
    scenarios. This preserves field-specific CAPEX/OPEX and other base values
    when a different parameter is being varied.
    """
    ...
    aliases = {
        ...
        "Inflation rate (%)": "Inflation Rate",
        "injection_rate_mtpa": "CO2 Injection Rate",
        "CO2 Injection Rate": "CO2 Injection Rate",
        "CO₂ Injection rate (MtCO₂/year)": "CO2 Injection Rate",
        "project_lifetime": "Project Lifetime",
        "Project Lifetime": "Project Lifetime",
        "Project lifetime (years)": "Project Lifetime",
        "carbon_credit": "Carbon Credits",
        "Carbon Credits": "Carbon Credits",
        "Carbon credits ($/tCO₂)": "Carbon Credits",
        "carbon_price": "Carbon Price",
        "Carbon Price": "Carbon Price",
        "Carbon price ($/tCO₂)": "Carbon Price",
        "government_subsidy": "Government Subsidy",
        "Government Subsidy": "Government Subsidy",
        "Government subsidy ($/tCO₂)": "Government Subsidy",
        "tax_incentive": "Tax Incentive",
        "Tax Incentive": "Tax Incentive",
        "Tax incentive ($/tCO₂)": "Tax Incentive",
        "storage_fee": "Storage Fee",
        "Storage Fee": "Storage Fee",
        "Storage fee ($/tCO₂)": "Storage Fee",
    }
    allowed = {
        "CAPEX", "OPEX", "Discount Rate", "Inflation Rate",
        "CO2 Injection Rate", "Project Lifetime", "Carbon Credits",
        "Carbon Price", "Government Subsidy", "Tax Incentive", "Storage Fee"
    }
    canonical_ranges = {}
    for raw_key, raw_range in ranges.items():
        key = aliases.get(raw_key, raw_key)
        if key not in allowed:
            raise ValueError(f"Unsupported sensitivity parameter: {raw_key}")
        if raw_range is None or len(raw_range) != 2:
            raise ValueError(f"Sensitivity range for {raw_key} must contain Low and High values.")
        low, high = float(raw_range[0]), float(raw_range[1])
        if not np.isfinite(low) or not np.isfinite(high):
            raise ValueError(f"Sensitivity range for {raw_key} contains a non-finite value.")
        if low > high:
            raise ValueError(f"Sensitivity Low value cannot exceed High value for {raw_key}.")
        if key == "Project Lifetime":
            low, high = max(1, int(round(low))), max(1, int(round(high)))
        canonical_ranges[key] = (low, high)

    parameter_order = [
        "CAPEX", "OPEX",
        "Carbon Price", "Carbon Credits", "Government Subsidy",
        "Tax Incentive", "Storage Fee",
        "Discount Rate", "Inflation Rate",
        "CO2 Injection Rate", "Project Lifetime"
    ]
    active_parameters = [p for p in parameter_order if p in canonical_ranges]

    for field in fields:
        scenarios = []
        base_field_capex = details[field].get("Actual CAPEX")
        base_field_opex = details[field].get("Actual OPEX")
        if base_field_capex is None or base_field_opex is None:
            raise ValueError(f"{field}: actual CAPEX and OPEX are required for sensitivity analysis.")

        scenario_no = 0
        for parameter in active_parameters:
            low, high = canonical_ranges[parameter]
            for label, value in (("Low", low), ("High", high)):
                scenario_no += 1
                a = dict(base_assumptions)
                actual_capex = float(base_field_capex)
                actual_opex = float(base_field_opex)
                ...
                if parameter == "CAPEX":
                    ...
                    a["carbon_price"] = value
                elif parameter == "Government Subsidy":
                    a["government_subsidy"] = value
                elif parameter == "Tax Incentive":
                    a["tax_incentive"] = value
                elif parameter == "Storage Fee":
                    a["storage_fee"] = value

                npv, irr, payback, annual_revenue, _ = project_cashflows(
                    actual_capex, actual_opex,
                    a["carbon_credit"], a["government_subsidy"],
                    a["tax_incentive"], a["storage_fee"],
                    a["discount_rate"], a["inflation_rate"],
                ...
                scenarios.append({
                    ...
                    "Revenue (Year 1)": annual_revenue,
                    "NPV": npv,
                    "IRR (%)": None if irr is None else irr * 100.0,
                    "Payback Period (years)": payback,
                })

        sdf = pd.DataFrame(scenarios)
        scenario_tables[field] = sdf
        npv_vals = pd.to_numeric(sdf["NPV"], errors="coerce").dropna()
        irr_vals = pd.to_numeric(sdf["IRR (%)"], errors="coerce").dropna()
        pb_vals = pd.to_numeric(sdf["Payback Period (years)"], errors="coerce").dropna()
        rows.append({
            "Field": field,
            "Sensitivity NPV Min": npv_vals.min() if not npv_vals.empty else None,
            "Sensitivity NPV Max": npv_vals.max() if not npv_vals.empty else None,
    ...
    df = pd.DataFrame(rows)
    if not df.empty:
        df["_npv_rank"] = df["Sensitivity NPV Min"].rank(ascending=False, method="min", na_option="bottom")
        ...
        df = df.sort_values(
            ["Sensitivity Rank Score", "Sensitivity NPV Min"],
