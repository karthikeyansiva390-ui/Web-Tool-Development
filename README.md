...
Sensitivity analysis uses a one-at-a-time (OAT) approach. Each requested parameter is changed to its Low and High endpoint while all other economic assumptions remain at their field/base values. With seven parameters this produces up to 14 scenarios per field. Revenue, NPV, IRR and Payback Period are recalculated for every scenario, and the displayed Min/Max values are the actual extrema across those scenarios.

### Sensitivity-analysis scope

The Phase-2 sensitivity analysis uses one-at-a-time (OAT) analysis. Each selected parameter is changed between its Low and High values while all other economic assumptions remain at their base values. The sensitivity parameters are:
...
For each scenario the application recalculates Revenue, NPV, IRR and Payback Period. The final sensitivity table reports the actual minimum and maximum results observed across all OAT scenarios. Revenue-related parameters are therefore explicitly propagated into the cash-flow calculation rather than being displayed as unused sensitivity inputs.


### Phase-1 to Phase-2 eligibility rule
A field that fails any Phase-1 hard cut-off is permanently eliminated and is not displayed or selectable in Phase 2. Only fields with `Passed=True` in the Phase-1 screening results are eligible for Phase 2. A Phase-2 CAPEX/OPEX gate failure remains a separate condition and may still be selected for subsequent economic analysis as specified by the framework.

## Latest regression fix

The Phase-2 field-selection state is explicitly initialised and reset between Phase-1 runs. Only fields with `Passed=True` from Phase 1 are eligible for Phase 2, and stale Phase-2 selections are filtered against the current Phase-1 qualified list. This prevents eliminated Phase-1 fields from reappearing and prevents a missing `phase2_selected_fields` session-state key from causing an AttributeError.
