...
st.markdown(
    ...
    .stage-card b, .stage-card span, .stage-card div {
        color:#111827 !important;
    }
    .invalid-note {
        color:#b00020;
...
def init_state():
    defaults = {
        ...
        "field_input_mode": {},
        "field_upload_status": {},
        # Canonical Phase-2 field-selection state. Initialised here so the
        # Phase-2 multiselect can never reference a missing session-state key.
        "phase2_selected_fields": [],
    }
...
def reset_phase1_results():
    ...
    # A new Phase-1 run invalidates all previous Phase-2 field selections.
    st.session_state.phase2_selected_fields = []
...
def reset_phase2():
    for k in [
        ...
        if isinstance(st.session_state[k], pd.DataFrame):
            ...
            st.session_state[k] = {}
    # Keep Phase-2 selection synchronized with the current Phase-1 results.
    st.session_state.phase2_selected_fields = []
    st.session_state.phase2_stage = 1
...
def create_manual_field(name: str, reference: dict[str, Any]):
    ...
    st.session_state.field_input_mode[name] = "Manual Entry"
    st.session_state.field_upload_status.pop(name, None)
...
def _find_header_row(df: pd.DataFrame) -> int | None:
    """Find the row containing Parameter / Value in a template-style sheet."""
    for i in range(min(len(df), 12)):
        vals = [str(x).strip().casefold() for x in df.iloc[i].tolist()]
        if "parameter" in vals and "value" in vals:
            return i
    return None


def _clean_import_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.columns:
        n = norm_text(c)
        if n in {"parameter", "sub parameter", "sub-parameter", "name"}:
            rename[c] = "Parameter"
        elif n in {"value", "input value", "input"}:
            rename[c] = "Value"
        elif n in {"unit/type", "unit type", "unit", "type"}:
            rename[c] = "Unit/Type"
        elif n in {"category", "common parameter", "common category"}:
            rename[c] = "Category"
    df = df.rename(columns=rename)
    if "Parameter" not in df.columns or "Value" not in df.columns:
        raise ValueError("The field file must contain 'Parameter' and 'Value' columns. Excel sheets may use the same template as the sample field datasheet.")
    return df


def parse_field_upload(file_bytes: bytes, filename: str, reference: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    """Parse the sample field template from XLSX or a flexible CSV format.

    XLSX: expects the five category sheets plus Economics, but missing sheets are allowed.
    CSV: accepts Parameter/Value/Unit/Type and optionally Category. If Category is omitted,
         parameter names are matched against the reference database.
    """
    rows = []
    errors = []
    ...
    if suffix == "xlsx":
        ...
        for sheet in xls.sheet_names:
            ...
            try:
                table = _clean_import_table(raw)
            except ValueError:
                continue
            default_cat = sheet if sheet in CATEGORIES else None
            for _, r in table.iterrows():
                p = str(r.get("Parameter", "")).strip()
                if not p or is_missing(p):
                    continue
                rows.append({
                    "Category": str(r.get("Category", "")).strip() if not is_missing(r.get("Category", "")) else default_cat,
                    "Parameter": p,
                    "Value": "" if is_missing(r.get("Value", "")) else str(r.get("Value", "")).strip(),
                    "Unit/Type": "" if is_missing(r.get("Unit/Type", "")) else str(r.get("Unit/Type", "")).strip(),
                })
    elif suffix == "csv":
        raw = pd.read_csv(io.BytesIO(file_bytes), dtype=str, keep_default_na=False)
        table = _clean_import_table(raw)
        for _, r in table.iterrows():
            p = str(r.get("Parameter", "")).strip()
            if not p or is_missing(p):
                continue
            rows.append({
                "Category": str(r.get("Category", "")).strip() if "Category" in table.columns else "",
                "Parameter": p,
                "Value": str(r.get("Value", "")).strip(),
                "Unit/Type": str(r.get("Unit/Type", "")).strip() if "Unit/Type" in table.columns else "",
            })
    else:
        raise ValueError("Only XLSX and CSV field files are supported.")

    if not rows:
        raise ValueError("No Parameter/Value records were found in the uploaded field file.")

    # Reference lookup by normalized parameter, with category disambiguation.
    ref_lookup = {}
    for cat in CATEGORIES:
        for spec in reference["categories"][cat]:
            ref_lookup[(cat, norm_text(spec["parameter"]))] = spec
    by_param = {}
    for cat in CATEGORIES:
        for spec in reference["categories"][cat]:
            by_param.setdefault(norm_text(spec["parameter"]), []).append((cat, spec))

    values = {cat: {spec["parameter"]: "" for spec in reference["categories"][cat]} for cat in CATEGORIES}
    actual_capex, actual_opex = "", ""

    for row in rows:
        pnorm = norm_text(row["Parameter"])
        cat = row["Category"] if row["Category"] in CATEGORIES else None
        spec = None
        if cat:
            spec = ref_lookup.get((cat, pnorm))
        else:
            matches = by_param.get(pnorm, [])
            if len(matches) == 1:
                cat, spec = matches[0]
            elif len(matches) > 1:
                errors.append(f"{row['Parameter']}: parameter exists in more than one category; add a Category column to the CSV.")
                continue

        if pnorm == norm_text("Actual CAPEX"):
            actual_capex = row["Value"]
            continue
        if pnorm == norm_text("Actual OPEX"):
            actual_opex = row["Value"]
            continue

        if spec is None:
            errors.append(f"Unknown parameter: {row['Parameter']}")
            continue

        value = row["Value"]
        valid, msg = validate_manual_value(value, spec)
        if not valid:
            errors.append(f"{cat} → {row['Parameter']}: {msg}")
            continue
        values[cat][spec["parameter"]] = value

    # Economics values are validated numerically when supplied.
    for label, val in [("Actual CAPEX", actual_capex), ("Actual OPEX", actual_opex)]:
        if str(val).strip() and not is_missing(val) and parse_number_strict(val) is None:
            errors.append(f"Economics → {label}: invalid numerical value.")
...
def apply_uploaded_field(field_name: str, reference: dict[str, Any], uploaded_file) -> tuple[bool, list[str]]:
    try:
        parsed, errors = parse_field_upload(uploaded_file.getvalue(), uploaded_file.name, reference)
    except Exception as exc:
        return False, [str(exc)]
    if errors:
        return False, errors
    st.session_state.manual_fields[field_name] = parsed
    st.session_state.field_upload_status[field_name] = f"Imported successfully from {uploaded_file.name}"
    reset_phase1_results()
    return True, []


def render_file_upload_editor(field_name: str, reference: dict[str, Any]):
    st.markdown(
        '<div class="stage-card"><b>File Upload Mode</b><br>'
        'Upload one field datasheet for this field. Use the sample template: each category is a sheet with <b>Parameter | Value | Unit/Type</b>. The Economics sheet contains Actual CAPEX and Actual OPEX. CSV files may additionally include a Category column.</div>',
        unsafe_allow_html=True,
    )
    uploaded = st.file_uploader(
        f"Upload input data for {field_name}",
        type=["xlsx", "csv"],
        accept_multiple_files=False,
        key=f"field_file_{field_name}",
        help="Upload one field file at a time. The filename is retained as the field name unless you rename it.",
    )
    if uploaded is not None:
        if st.button("Import this field data", key=f"import_{field_name}", type="primary"):
            ok, errors = apply_uploaded_field(field_name, reference, uploaded)
            if ok:
                st.success(f"{uploaded.name} imported successfully for {field_name}.")
                st.rerun()
            else:
                st.error("The file was not imported. Correct the following issues:")
                for e in errors:
                    st.markdown(f"- {e}")
    if field_name in st.session_state.field_upload_status:
        st.success(st.session_state.field_upload_status[field_name])
...
if phase == "Phase 1 — Screening":
    ...
    st.info("For every field, choose how its Phase-1 input values will be provided. Manual Entry opens the existing tabbed form. File Upload accepts the sample Excel/CSV field-data structure.")
    ...
    mode = st.radio(
        f"How do you want to enter input values for {active_field}?",
        ["Manual Entry", "File Upload"],
        horizontal=True,
        key=f"input_mode_{active_field}",
        index=0 if st.session_state.field_input_mode.get(active_field, "Manual Entry") == "Manual Entry" else 1,
    )
    st.session_state.field_input_mode[active_field] = mode

    if mode == "Manual Entry":
        render_manual_field_editor(active_field, reference)
    else:
        render_file_upload_editor(active_field, reference)
...
# ===========================================================================
    ...
    # STRICT PHASE-1 ELIGIBILITY RULE:
    # Only fields that passed ALL Phase-1 hard-cutoff requirements may enter
    # Phase 2. Phase-1 eliminated fields must never be displayed or selectable
    # in the Phase-2 field-selection gate.
    phase1_qualified_fields = phase1_qualified_field_names(details)

    # Remove any stale Phase-2 selections left in session state from an older
    # run/version so eliminated Phase-1 fields cannot reappear.
    # Always initialise the widget state before reading it. Streamlit's
    # session_state raises AttributeError for a missing key, which was the
    # source of the Phase-2 crash in the previous build.
    if "phase2_selected_fields" not in st.session_state:
        st.session_state.phase2_selected_fields = []

    # Remove stale selections from previous Phase-1 runs. This also guarantees
    # that a Phase-1 eliminated field can never re-enter Phase 2 through stale
    # browser/session state.
    st.session_state.phase2_selected_fields = [
        f for f in st.session_state.phase2_selected_fields
        if f in phase1_qualified_fields
    ]

    st.subheader("Stage 1 — Select Phase-1 Qualified Fields")

    if not phase1_qualified_fields:
        st.error("No field passed Phase 1. All Phase-1 eliminated fields are excluded from Phase 2.")
        st.stop()
    ...
    if selection_mode == "Select all fields":
        selected_fields = list(phase1_qualified_fields)
        st.success(f"{len(selected_fields)} Phase-1 qualified field(s) selected.")
    else:
        selected_fields = st.multiselect(
            "Select one or more Phase-1 qualified fields",
            phase1_qualified_fields,
            default=st.session_state.phase2_selected_fields,
    ...
    if not gate_passed.empty or not gate_failed.empty:
        ...
        if economic_selection_mode == "Select all selected fields":
            ...
            economic_fields = st.multiselect(
                "Choose fields for Revenue / NPV / IRR / Payback",
        ...
        if st.button("Calculate Revenue, NPV, IRR & Payback Period", type="primary"):
            try:
        ...
        if not st.session_state.economic_df.empty:
            ...
            if do_sensitivity == "Go ahead":
                ...
                st.markdown(
                    "Enter **low and high** values. The framework performs one-at-a-time sensitivity analysis: each parameter is changed to its Low and High value while the other economic assumptions remain at their base values. Revenue, NPV, IRR and Payback Period are recalculated for every scenario."
                )
                ...
                sensitivity_specs = [
                    ("CAPEX (USD)", "capex"),
                    ("OPEX (USD/Year)", "opex"),
                    ("Carbon price ($/tCO₂)", "carbon_price"),
                    ("Carbon credits ($/tCO₂)", "carbon_credit"),
                    ("Government subsidy ($/tCO₂)", "government_subsidy"),
                    ("Tax incentive ($/tCO₂)", "tax_incentive"),
                    ("Storage fee ($/tCO₂)", "storage_fee"),
