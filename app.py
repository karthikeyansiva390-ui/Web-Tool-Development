...
# ===========================================================================
    ...
    # IMPORTANT FRAMEWORK RULE: a field that fails the Phase-1 hard-cutoff
    # screening is permanently eliminated and MUST NOT enter Phase 2.
    # Phase-2 CAPEX/OPEX gate failures are different: those may remain
    # selectable later, as specified by the framework.
    all_fields = phase1_qualified_fields(details)

    st.subheader("Stage 1 — Select Fields")

    if not all_fields:
        st.warning("No field passed the Phase-1 hard cut-off screening. Phase 2 is therefore unavailable.")
        st.stop()
    ...
    if not gate_passed.empty or not gate_failed.empty:
        ...
        if not gate_failed.empty:
            st.warning(
                "These fields failed the Phase-2 CAPEX/OPEX gate. They may still be selected for further economic analysis, as permitted by the framework."
            )
            st.dataframe(gate_failed, use_container_width=True, hide_index=True)

        # Phase-2 CAPEX/OPEX gate failures may continue to economic analysis
        # if selected. This does NOT re-admit any Phase-1 failed field because
        # the Phase-2 selection list was already restricted above.
        ...
        if not st.session_state.economic_df.empty:
            ...
            if do_sensitivity == "Go ahead":
                ...
                st.markdown(
                    "Enter **low and high** values. The framework will rerun NPV, IRR and Payback Period for the requested sensitivity range. Revenue is recalculated automatically from the same injection rate and revenue assumptions."
                )
